...
async def cmd_status(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        f"⚙️  Active job    : {'🔄 Running' if active else '✅ Free'}\n"
...
async def cmd_leads(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if _active_jobs.get(uid):
        await update.message.reply_text(
            "⚠️ You already have an active job running. Wait for it to finish."
        )
        return
...
async def cmd_outreach(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if _active_jobs.get(uid):
        await update.message.reply_text(
            "⚠️ A job is already running. Check /status for progress."
        )
        return
...
async def cmd_import(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    uid = update.effective_user.id
    if _active_jobs.get(uid):
        await update.message.reply_text("⚠️ A job is already running. Wait for it to finish.")
        return
