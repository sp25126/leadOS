...
async def run_enrichment(
    ...
    """
    # ── BYOK Authentication (if integrated into enricher later) ──
    snov_key = request.headers.get("X-Snov-Key") or os.getenv("SNOV_CLIENT_ID")
    snov_secret = request.headers.get("X-Snov-Secret") or os.getenv("SNOV_CLIENT_SECRET")
    getprospect_key = request.headers.get("X-GetProspect-Key") or os.getenv("GETPROSPECT_API_KEY")
    
    # We would pass these down to enrich_all if the python logic supported them.
    # Currently enricher.py uses basic web scraping, so we just set them in env.
    if snov_key: os.environ["SNOV_CLIENT_ID"] = snov_key
    if snov_secret: os.environ["SNOV_CLIENT_SECRET"] = snov_secret
    if getprospect_key: os.environ["GETPROSPECT_API_KEY"] = getprospect_key
    ...
    # Run enrichment
    enriched_leads = await enrich_all(leads_to_enrich, concurrency=5)
