...
async def run_search_background(task_id: str, req: LeadSearchRequest, gemini_key: str):
    try:
        ...
        raw_leads = await discover_leads(req.business_type, req.location, req.radius_km)
        
        # Defensive check: Ensure raw_leads is a list of dicts
        if not isinstance(raw_leads, list):
            print(f"⚠️ [Task {task_id}] Discovery returned non-list result. Converting to empty list.")
            raw_leads = []
        raw_leads = [l for l in raw_leads if isinstance(l, dict)]
        
        print(f"✅ [Task {task_id}] Discovery complete. Found {len(raw_leads)} potential leads.")

        if not raw_leads:
            print(f"ℹ️ [Task {task_id}] No leads found. Ending task.")
        ...
        # Stage 2: Enrich
        print(f"✨ [Task {task_id}] Stage 2/4: Enriching lead data (emails, social, etc)...")
        _search_store[task_id]["status"] = "enriching"
        _search_store[task_id]["progress"] = 50
        enriched = await enrich_all(raw_leads, concurrency=5)
        enriched = [l for l in enriched if isinstance(l, dict)]

        # Stage 3: Score
        print(f"🤖 [Task {task_id}] Stage 3/4: AI Scoring & Personalisation...")
        _search_store[task_id]["status"] = "scoring"
        _search_store[task_id]["progress"] = 75
        scored = await score_leads(enriched, req.target_service, gemini_key=gemini_key)
        ...
        _search_store[task_id]["status"] = "completed"
        _search_store[task_id]["progress"] = 100
        _search_store[task_id]["result"] = result
        print(f"🏁 [Task {task_id}] HUNT SUCCESSFUL. Session: {session_id}")
        print(f"─" * 60)

    except Exception as e:
        print(f"❌ [Task {task_id}] HUNT FAILED: {e}")
...
async def search_leads(
    ...
    background_tasks.add_task(run_search_background, task_id, req, gemini_key)
...
async def get_all_leads(
    ...
    """Phase 3 endpoint: Get all leads from CSVs for the CRM table."""
    os.makedirs(RESULTS_DIR, exist_ok=True)
    all_leads = []
    
    # Collect and sort CSV files by modification time (newest first)
    csv_files = []
