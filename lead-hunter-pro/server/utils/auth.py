...
async def verify_api_key(request: Request):
    ...
    if not key or key != authoritative:
        safe_key = key[:4] if key else "None"
        safe_auth = authoritative[:4] if authoritative else "None"
        print(f"  [AUTH] Validation Failed. Received: {safe_key}... Expected: {safe_auth}...")
        raise HTTPException(
            status_code=401,
