    ...
    if result and isinstance(result, str):
        ...
        try:
            ...
            if isinstance(items, list) and len(items) > 0:
                ...
                print(f"  [WARN] Geocoding returned unexpected format: {str(items)[:80]}")
...
async def discover_leads(
    ...
    try:
        ...
        print(f"  [LOC] Geocoded '{location}' -> ({lat:.4f}, {lon:.4f})")
    except ValueError as e:
        print(f"  [ERROR] {e} -- OSM and HERE will be skipped")

    #   Source 1: OSM Overpass (3 mirrors)  
    print(f"\n  [MAP] [OSM Overpass]   Searching...")
    if lat and lon and any(quota.has_quota(k) for k in
        ...
        print("  [OSM Overpass]       Quota exhausted or no coordinates")

    if len(all_leads) >= MIN_RESULTS:
        print(f"  [OK] Sufficient results ({len(all_leads)}) -- skipping remaining sources")
        return _finalize(all_leads)

    #   Source 2: Google Maps  
    print(f"  [G] [Google Maps]     Searching...")
    if quota.has_quota("google_maps"):
        gmaps = await search_google_places(business_type, location, radius_m, api_key=gmaps_key)
        all_leads.extend(gmaps)
        _print_source_result("Google Maps", len(gmaps), quota.remaining("google_maps"))
    else:
        print("  [Google Maps]     [WARN] Monthly quota exhausted")
    ...
    if quota.has_quota("here_places"):
        ...
        print("  [HERE Places]        Daily quota exhausted - resets at midnight")
